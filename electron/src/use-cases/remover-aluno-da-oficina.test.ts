...
import { removerAlunoDaOficina } from "./remover-aluno-da-oficina";

describe("remove aluno da oficina", () => {
  test("remove aluno", async () => {
    ...
    oficinaCriada.alunos = [alunoCriado];

    await oficinaRepo.save(oficinaCriada);

    const resposta = await removerAlunoDaOficina(
      alunoCriado.id,
      oficinaCriada.id
    );
    ...
    expect(oficinaAtualizada.alunos.length).toBe(0);
  });
  test("remove apenas o aluno e mantem os já existentes", async () => {
    const aluno = criaAluno();
    ...
    oficinaCriada.alunos = [outroAluno, alunoCriado];
    ...
    expect(oficinaAtualizada.alunos[0].id).toBe(outroAluno.id);
  });
});
